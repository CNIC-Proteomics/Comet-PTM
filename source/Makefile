...
# EXECNAME = comet.exe
EXECNAME = comet-iq.exe
EXECNAME_DEBUG = comet_debug.exe
OBJS = Comet.o
...
# LIBS_DEBUG = -lcometsearch_debug -lmstoolkitlite -lm -lpthread
LIBS_DEBUG = -lcometsearch_debug -lmstoolkitlite_debug -lm -lpthread
ifdef MSYSTEM
   LIBS += -lws2_32
   LIBS_DEBUG += -lws2_32
endif
...
debug: $(OBJS)
	cd $(MSTOOLKIT) ; make debug ; cd ../CometSearch ; make debug
	${CXX} $(CXXFLAGS) $(OBJS) $(LIBPATHS) $(LIBS_DEBUG) -o ${EXECNAME_DEBUG}
