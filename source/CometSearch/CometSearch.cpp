...
// spiros
         ...
         //   bTrimDescr = 1;
         if (isspace(iTmpCh) || iTmpCh == 44/*comma ','*/) //|| iscntrl(iTmpCh))
         {
            iTmpCh = 95;   // ascii for underscore '_'
         }
         else if (iscntrl(iTmpCh))
            bTrimDescr = 1;
...
// spiros comet-ptm: don´t calculate non modified xcorr when delta mass not near 0
                  double deltaXcorrMass = g_pvQuery.at(iWhichQuery)->_pepMassInfo.dExpPepMass - dCalcPepMass;
                  bool bDeltaXcorrSearch = g_staticParams.options.bUseDeltaXcorr && g_staticParams.options.bDontCalcPseudoNonMod &&
                     (deltaXcorrMass < -g_staticParams.options.dDeltaInnerTol || deltaXcorrMass > g_staticParams.options.dDeltaInnerTol);
...
// SPIROS START comet-ptm
                  ...
                  DeltaXcorrSearch(deltaXcorrMass, iWhichQuery, dCalcPepMass, iEndPos, iStartPos, iProteinSeqLengthMinus1, 
                                   szProteinSeq, pbDuplFragment, szProteinName, iLenPeptide);
                  ...
                  if (g_staticParams.options.bUseDeltaBackJumps)
                  {
                     for (int iSaltito = 1; iSaltito < g_massRange.iMaxFragmentCharge; ++iSaltito)
                     {
                        ...
                        DeltaXcorrSearch(deltaXcorrMass, iWhichQuery, dCalcPepMass, iEndPos, iStartPos, iProteinSeqLengthMinus1,
                                         szProteinSeq, pbDuplFragment, szProteinName, iLenPeptide, "-", iSaltito);
                     }
                  }
...
// spiros end comet-ptm
{
   ...
   unsigned int (*p_uiBinnedIonMassesDelta)[MAX_FRAGMENT_CHARGE + 1][9][MAX_PEPTIDE_LEN]; // spiros comet-ptm
   ...
   else
   {
      ...
      p_uiBinnedIonMassesDelta = &_uiBinnedIonMassesDelta;	// spiros comet-ptm
   }
      ...
      for (ctIonSeries = 0; ctIonSeries < g_staticParams.ionInformation.iNumIonSeriesUsed; ctIonSeries++)
      {
         ...
         if (iWhichIonSeries == ION_SERIES_B)
         {
            for (int i = bestPos; i < iLenPeptideMinus1; ++i)   // b series
            {
               for (ctCharge = 1; ctCharge <= pQuery->_spectrumInfoInternal.iMaxFragCharge; ctCharge++)
               {
                  if (vctXcorrs[ctCharge - 1][ctIonSeries][DELTA_XCORR_ID][i] > vctXcorrs[ctCharge - 1][ctIonSeries][XCORR_ID][i])   // DeltaXcorr > Xcorr
                  {
...
// comet-ptm spiros end
   ...
   else
   {
      ...
      {
         if (!CheckDuplicate(iWhichQuery, iStartPos, iEndPos, bFoundVariableMod, dCalcPepMass,
               szProteinSeq, szProteinName, 0, pcVarModSites, bDeltaXcorrSearch, dDeltaXcorrMass))
         {
            StorePeptide(iWhichQuery, iStartPos, iEndPos, bFoundVariableMod, szProteinSeq,
                  dCalcPepMass, dXcorr, szProteinName, 0, pcVarModSites, bDeltaXcorrSearch, dDeltaXcorrMass, vctBitMods, sDeltaJumps, bestPos);
         }
      }      
   }
...
int CometSearch::CheckDuplicate(int iWhichQuery,
                                ...
                                bool bDecoyResults,
                                char *pcVarModSites, 
                                bool bDeltaXcorrSearch /*= false*/,	// spiros comet-ptm
                                double dDeltaXcorrMass /*= 0.0*/)	// spiros comet-ptm
{
   int i,
       iLenMinus1,
       bIsDuplicate=0;
...
// spiros comet-ptm start
void CometSearch::DeltaXcorrSearch(const double& dDeltaXcorrMass, const int& iWhichQuery, const double& dCalcPepMass,
   const int& iEndPos, const int& iStartPos, const int& iProteinSeqLengthMinus1,
   char* szProteinSeq, bool* pbDuplFragment, char* szProteinName,
   const int& iLenPeptide, std::string sSaltito /*= "0"*/, int iSaltito /*= 0*/)
{
   ...
   if (bDeltaXcorrSearch)
   {
      ...
      if (!g_staticParams.variableModParameters.bRequireVarMod)
      {
         ...
         XcorrScore(szProteinSeq, szProteinName, iStartPos, iEndPos, false, dCalcPepMass, false, iWhichQuery, iLenPeptide, 
                    pcVarModSites, bDeltaXcorrSearch, dDeltaXcorrMass, sSaltito);
      }
