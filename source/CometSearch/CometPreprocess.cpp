                                 ...
                                 double *pdTmpPeakExtracted)
{
   ...
   struct msdata pTmpQData[NUM_SP_IONS];
   ...
   if (!CNICLoadIons(pScoring, pdTmpRawData, pdQTmpRawData, mstSpectrum, &pPre))
   {
      return false;
   }
...
// spiros
   ...
   CNICMakeCorrData(pdTmpRawData, pdQTmpRawData, pdTmpCorrelationData, pdQTmpCorrelationData, pScoring, &pPre);
   ...
   {
      pTmpSpData[i].dIon = pTmpQData[i].dIon = 0.0;
      pTmpSpData[i].dIntensity = pTmpQData[i].dIntensity = 0.0;
   }
   ...
   for (z=0; z<zStop; z++)
   {
                     ...
                     || (iPrecursorCharge == 1 && g_staticParams.options.bOverrideCharge == 3)))))
      {
               ...
               && iPrecursorCharge <= g_staticParams.options.iMaxPrecursorCharge)
         {
            ...
            if (!Preprocess(pScoring, spec, pdTmpRawData, pdQTmpRawData, pdTmpFastXcorrData,
                     pdTmpCorrelationData, pdQTmpCorrelationData, pdTmpSmoothedSpectrum, pdTmpPeakExtracted))
            {
               return false;
            }
...
//{
//// spiros
//   //std::ofstream loadIonsFile;
//   //loadIonsFile.open("loadIonsFile.csv", ios::out | ios::app | ios::ate);
//   //loadIonsFile << "i, dIon, dIntensity" << std::endl;
//
//   int  i;
//   double dIon,
//          dIntensity;
...
// spiros: 12/11/2015 Modified from loadIons() to handle QScore
bool CometPreprocess::CNICLoadIons(struct Query *pScoring, double *pdTmpRawData, double *pdQTmpRawData, 
         Spectrum mstSpectrum, struct PreprocessStruct *pPre)
{
   // spiros
   //std::ofstream loadIonsFile;
   //loadIonsFile.open("loadIonsFile.csv", ios::out | ios::app | ios::ate);
   ...
   int  i;
   double dIon, dIntensity, dQIntensity;
...
void CometPreprocess::CNICMakeCorrData(double *pdTmpRawData,
                                       double *pdQTmpRawData,
                                       double *pdTmpCorrelationData,
                                       double *pdQTmpCorrelationData,
                                       struct Query *pScoring,
                                 ...
                                 int iArraySize)
{
   ...
   for (i=0; i<iArraySize; i++)
   {
      if (pdTmpRawData[i] > dLowestInten)
      {
         ...
         (pTmpSpData+iLowestIntenIndex)->dIon = (pTmpQData + iLowestIntenIndex)->dIon = (double)i;
