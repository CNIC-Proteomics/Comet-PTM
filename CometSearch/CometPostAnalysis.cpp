                                    ...
                                    int iSize)
{
   ...
   for (i=0; i<iSize; i++)
   {
      ...
      //spiros: new QScore for (ctCharge=1; ctCharge<=iMaxFragCharge; ctCharge++)
...
int CometPostAnalysis::QSortFnQ(const void *a,
   const void *b)
{
   struct Results *ia = (struct Results *)a;
   struct Results *ib = (struct Results *)b;
...
float CometPostAnalysis::FindQScore(Query *pQuery,
                                    int bin)
{
   int x = bin / SPARSE_MATRIX_SIZE;
